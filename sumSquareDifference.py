...
# The square of sums of of a set of nummbers, say a,b,c, and d, is equal to the sum of squares (call it f) plus
# two times the sum of all permutations of two numbers (e.g., the square of sums is equal to f + 2(ab + ac + ad + bc + bd + cd)
# so the difference is 2(ab + ac + ad + bc + bd + cd))

# for the set nums=[a,b,c,d], this method ultimately returns ((ab+ac+ad)+(bc+bd)+(cd))
def sumPermutations(nums):
    retval = 0;
    if(len(nums) > 2):
        head = nums[0]
        tail = nums[1:]
        ...
        for n in tail:
            sum += head*n
        return sum + sumPermutations(tail)
    elif(len(nums) == 2):
